...
public class WordParser {
    ...
    // words needs to start with a letter
    private static final String FILTER_REGEX = "[A-Za-z][^\\s!.,;:/=@]+";


    /**
     * count how often a word occurs inside an array with words
     * @param arr array with words
     * @param word word to count
     * @return
     */
    public static int countWords(String[] arr, String word) {
        int count = 0;
        for (Object o : arr) {
            //o nicht arr, oder?
            if (o.equals(word)) {
    ...
    public static String getWordStem(String word) {
        ...
        if (USE_STEMMING && word.matches("[a-zA-Z]+")) {
            // transform our word to the root form
            String temp = Stemmer.readString(word).get(0);
            if (temp.length() > 5) {
                return temp;
            }
        }
     ...
     * @return
     */
    public static String[] getFilteredWords(String s) {
        String[] words = s.split("\\s+");
        List<String> wordList = new ArrayList<>();
        for (int i = 0; i < words.length; i++) {
            if (words[i].matches(FILTER_REGEX)) {
                wordList.add(getWordStem(words[i]));
            }
        }
